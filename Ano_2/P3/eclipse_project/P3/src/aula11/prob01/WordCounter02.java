...
public class WordCounter02 {
	
	private Map<String, Integer>  wordCount;
	public int differentWords;
	public int totalWords;
	
	public WordCounter02(String filePath) throws IOException {
	...
	public void countWords(String filename) throws IOException {
		...
		List<String> lines = Files.readAllLines(Paths.get(filename));
		lines.stream().flatMap(line -> Arrays.stream(line.split(" "))).forEach(word -> { differentWordsSet.add(word); totalWords++; });
	...
	public String mapToString() { return wordCount.keySet().stream().map(key -> key+"\t"+wordCount.get(key)).collect(Collectors.joining("\n")); }
